  ...
  return (
    <>
      ...
      <div className="text-center my-5">
        <button className="btn btn-info mx-3 " style={{ fontWeight: 'bold', fontSize: '1.2rem' }}
        onClick={()=> navigate('/')}
        >Cart is empty!! Click here continue shopping...</button>
        </div>
      </>) : (
        <>
      ...
      {cart?.items?.length > 0 && (
        <div className="container text-center">
          <button className="btn btn-warning mx-5 " style={{ fontWeight: 'bold' }}
          onClick={()=>navigate("/shipping")}
          >Checkout</button>
          <button className="btn btn-danger mx-5 " style={{ fontWeight: 'bold' }}
            onClick={()=>{
              if(confirm("Are you sure?")) {
                clearCart();
              }
